class fileReader {
    ...
    setVertex(line): void {
        ...
        if (this.justOne(+x1, +x2, +y1, +y2, +z1, +z2) && this.justOne(+x1, +x3, +y1, +y3, +z1, +z3)) {
            ...
            this.actualAdd(this.moveSize(+x1, +x2, +x3), this.moveSize(+y1, +y2, +y3), this.moveSize(+z1, +z2, +z3), this.GvtArray[(+t1 - 1)] + answer, Math.round(+x1) - this.lowX, Math.round(+y1) - this.lowY, Math.round(+z1) - this.lowZ);
            ...
            this.actualAdd(this.moveSize(+x2, +x1, +x3), this.moveSize(+y2, +y1, +y3), this.moveSize(+z2, +z1, +z3), this.GvtArray[(+t2 - 1)] + answer, Math.round(+x2) - this.lowX, Math.round(+y2) - this.lowY, Math.round(+z2) - this.lowZ);
            ...
            this.actualAdd(this.moveSize(+x3, +x2, +x1), this.moveSize(+y3, +y2, +y1), this.moveSize(+z3, +z2, +z1), this.GvtArray[(+t3 - 1)] + answer, Math.round(+x3) - this.lowX, Math.round(+y3) - this.lowY, Math.round(+z3) - this.lowZ);
            ...
            this.stupidityCatcher(x1, x2, x3, y1, y2, y3, z1, z2, z3, t1, t2, t3);
    }

    stupidityCatcher(x1, x2, x3, y1, y2, y3, z1, z2, z3, t1, t2, t3): void {
        //alert("alert1");
        var answer;
        if (this.justOne(+x1, +x2, +y1, +y2, +z1, +z2) && this.justOne(+x1, +x3, +y1, +y3, +z1, +z3)) {
            answer = this.concatonateDirection(this.whichWay(+x1, +x2, +y1, +y2, +z1, +z2), this.whichWay(+x1, +x3, +y1, +y3, +z1, +z3));
        ...
        if (this.convertToSegment(x1, y1, z1) != this.convertToSegment(x2, y2, z2) && this.convertToSegment(x1, y1, z1) != this.convertToSegment(x3, y3, z3) && this.convertToSegment(x2, y2, z2) != this.convertToSegment(x3, y3, z3)) {
            if (x1 != x2 && x1 != x3 && x1 != x2) {
                if (+x1 == this.getLowest(+x1, +x2, +x3))
                    x1 = +x1 + 1;
                if (+x1 == this.getHighest(+x1, +x2, +x3))
                    x1 = +x1 - 1;
                if (+x2 == this.getLowest(+x1, +x2, +x3))
                    x2 = +x2 + 1;
                if (+x2 == this.getHighest(+x1, +x2, +x3))
                    x2 = +x2 - 1;
                if (+x3 == this.getLowest(+x1, +x2, +x3))
                    x3 = +x3 + 1;
                if (+x3 == this.getHighest(+x1, +x2, +x3))
                    x3 = +x3 - 1;
            }
            if (y1 != y2 && y1 != y3 && y1 != y2) {
                if (+y1 == this.getLowest(+y1, +y2, +y3))
                    y1 = +y1 + 1;
                if (+y1 == this.getHighest(+y1, +y2, +y3))
                    y1 = +y1 - 1;
                if (+y2 == this.getLowest(+y1, +y2, +y3))
                    y2 = +y2 + 1;
                if (+y2 == this.getHighest(+y1, +y2, +y3))
                    y2 = +y2 - 1;
                if (+y3 == this.getLowest(+y1, +y2, +y3))
                    y3 = +y3 + 1;
                if (+y3 == this.getHighest(+y1, +y2, +y3))
                    y3 = +y3 - 1;
            }
            if (z1 != z2 && z1 != z3 && z1 != z2) {
                if (+z1 == this.getLowest(+z1, +z2, +z3))
                    z1 = +z1 + 1;
                if (+z1 == this.getHighest(+z1, +z2, +z3))
                    z1 = +z1 - 1;
                if (+z2 == this.getLowest(+z1, +z2, +z3))
                    z2 = +z2 + 1;
                if (+z2 == this.getHighest(+z1, +z2, +z3))
                    z2 = +z2 - 1;
                if (+z3 == this.getLowest(+z1, +z2, +z3))
                    z3 = +y3 + 1;
                if (+z3 == this.getHighest(+z1, +z2, +z3))
                    z3 = +z3 - 1;
            }
    ...
    convertToSegment(x1, y1, z1): string {
        return x1 + "/" + y1 + "/" + z1;
    }

    convertToLine(x1, x2, x3, y1, y2, y3, z1, z2, z3): string {
        return x1 + "/" + y1 + "/" + z1 + " " + x2 + "/" + y2 + "/" + z2 + " " + x3 + "/" + y3 + "/" + z3;
    }

    actualAdd(xJump, yJump, zJump, answer, xStart, yStart, zStart): void {
        var x = xStart;
        var y = yStart;
        var z = zStart;

        //alert("In actual add X: " + xJump + " Y: " + yJump + " Z: " + zJump + "  " + " sX: " + xStart + " sY: " + yStart + " sZ: " + zStart + "  " + answer);
        if (xJump > 0) {//if xJump is > 0 loop start
            for (var x = xStart; x < xJump + xStart; x++) {//xJump is > 0 loop start
                if (yJump > 0) {//if yJump is > 0 loop start
                    for (var y = yStart; y < yJump + yStart; y++) {//yJump is > 0 loop start
                        if (zJump > 0) {//if zJump is > 0 loop start
                            for (var z = zStart; z < zJump + zStart; z++) {
                                //alert("Adding: " + answer + " at " + x + "/" + y + "/" + z);
                                if (this.matrix[x][y][z] != "")
                                    this.matrix[x][y][z] += "/" + answer;//if texture is incorrect then change this.GvtArray[+t1]
                                else
                                    this.matrix[x][y][z] = answer;
                            }
                        }//if zJump is > 0 loop end
                        if (zJump < 0) {//if zJump is < 0 loop start
                            for (var z = zStart; z > zStart + zJump; z--) {
                                //alert("Adding: " + answer + " at " + x + "/" + y + "/" + z);
                                if (this.matrix[x][y][z] != "")
                                    this.matrix[x][y][z] += "/" + answer;//if texture is incorrect then change this.GvtArray[+t1]
                                else
                        ...
                        if (zJump == 0) {//if zJump is = 0 start
                            //alert("Adding: " + answer + " at " + x + "/" + y + "/" + z);
                            if (this.matrix[x][y][z] != "")
                                this.matrix[x][y][z] += "/" + answer;//if texture is incorrect then change this.GvtArray[+t1]
                            else
                ...
                }//if yJump is > 0 loop end
                if (yJump < 0) {//if yJump is < 0 loop start
                    for (var y = yStart; y > yStart + yJump; y--) {//yJump is < 0 loop start
                        if (zJump > 0) {//if zJump is > 0 loop start
                            for (var z = zStart; z < zJump + zStart; z++) {
                                //alert("Adding: " + answer + " at " + x + "/" + y + "/" + z);
                                if (this.matrix[x][y][z] != "")
                ...
                if (yJump == 0) {//if yJump is = 0 start
                    if (zJump > 0) {//if zJump is > 0 loop start
                        for (var z = zStart; z < zJump + zStart; z++) {
                            //alert("Adding: " + answer + " at " + x + "/" + y + "/" + z);
                            if (this.matrix[x][y][z] != "")
        ...
        }//if xJump is > 0 loop end
        else if (xJump < 0) {//if xJump is > 0 loop start
            for (var x = xStart; x > xStart + xJump; x--) {//xJump is > 0 loop start
                if (yJump > 0) {//if yJump is > 0 loop start
                    for (var y = yStart; y < yJump + yStart; y++) {//yJump is > 0 loop start
                        if (zJump > 0) {//if zJump is > 0 loop start
                            for (var z = zStart; z < zJump + zStart; z++) {
                ...
                if (yJump < 0) {//if yJump is < 0 loop start
                    for (var y = yStart; y > yStart + yJump; y--) {//yJump is < 0 loop start
                        if (zJump > 0) {//if zJump is > 0 loop start
                            for (var z = zStart; z < yStart + zJump; z++) {
                                //alert("Adding: " + answer + " at " + x + "/" + y + "/" + z);
                                if (this.matrix[x][y][z] != "")
                                    this.matrix[x][y][z] += "/" + answer;//if texture is incorrect then change this.GvtArray[+t1]
                                else
    ...
    moveSize(f1, s1, s2): number {
        //alert("f1: " + f1 + " s1: " + s1 + " s2: " + s2);
        if (f1 < s1 && f1 < s2)
            return f1 - s1;
        if (f1 > s1 && f1 > s2)
            return f1 - s1;
    ...
    isGood(line, position): boolean {
        ...
        if (position == 0) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("0.Comparing +_+_0 || 0_+_+ || +_0_+ to " + arrowArray[f] + " from " + line);
        ...
        if (position == 1) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("1.Comparing 0_+_- || +_+_0 || +_0_- to " + arrowArray[f] + " from " + line);
        ...
        if (position == 2) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("2.Comparing -_0_+ || -_+_0 || 0_+_+ to " + arrowArray[f] + " from " + line);
        ...
        if (position == 3) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("3.Comparing 0_+_- || -_+_0 || -_0_- to " + arrowArray[f] + " from " + line);
                if (arrowArray[f] == "0_+_-" || arrowArray[f] == "-_+_0" || arrowArray[f] == "-_0_-")
                    return true;
            }
        }
        if (position == 4) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("4.Comparing 0_-_+ || +_-_0 || +_0_+ to " + arrowArray[f] + " from " + line);
                if (arrowArray[f] == "0_-_+" || arrowArray[f] == "+_-_0" || arrowArray[f] == "+_0_+")
                    return true;
            }
        }
        if (position == 5) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("5.Comparing +_0_- || +_-_0 || 0_-_- to " + arrowArray[f] + " from " + line);
                if (arrowArray[f] == "+_0_-" || arrowArray[f] == "+_-_0" || arrowArray[f] == "0_-_-")
                    return true;
            }
        }
        if (position == 6) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("6.Comparing 0_-_+ || -_-_0 || -_0_+ to " + arrowArray[f] + " from " + line);
                if (arrowArray[f] == "0_-_+" || arrowArray[f] == "-_-_0" || arrowArray[f] == "-_0_+")
                    return true;
            }
        }
        if (position == 7) {
            for (var f = 0; f < arrowArray.length; f++) {
                //alert("7.Comparing -_-_0 || -_0_- || 0_-_- to " + arrowArray[f] + " from " + line);
